...
def train(H, test_images):
    ...
    with open(H['save_dir'] + '/hypes.json', 'w') as f:
        json.dump(H, f, indent=4) #Savings settings in json format

    x_in = tf.placeholder(tf.float32) #Store the data
    confs_in = tf.placeholder(tf.float32) #Store the configs?
    boxes_in = tf.placeholder(tf.float32) #Store the boxes
    q = {}
    enqueue_op = {}
    for phase in ['train', 'test']:
...
def main():
    ...
    parser.add_argument('--test', required=False, nargs='*', type=str)
